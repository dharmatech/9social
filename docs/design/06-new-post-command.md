...
Implementation note: the common draft-to-post publishing behavior should live in `bin/9social/lib/draft/publish` so `9social/cmd/new-post` and the Acme `9social/Draft/Publish` command use the same validation, metadata generation, filename selection, final write, Git commit, success cleanup, and failure preservation rules. `new-post` is responsible for creating the draft and either opening the editor or reading stdin before invoking the helper.
