...
This gives automated tests and terminal workflows a non-Acme way to create reply posts. The Acme `9social/Post/Reply` command uses the same `draft/reply` helper, then opens the draft in an editable Acme window instead of publishing immediately.
